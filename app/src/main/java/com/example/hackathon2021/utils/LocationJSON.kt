package com.example.hackathon2021.utils
...
object LocationsJSON {
    ...
    fun getLocations(query: String, radius: Int): List<List<String>> {
        // Use query parameter as description.
        // Radius parameter should be in miles (converted to meters for API call).
    ...
    private fun getDetails(placeID: String, query: String): MutableList<String> {
        ...
        itemData.add("${String.format("%.2f", calcDistance(latitude, longitude, loc.getString("lat").toDouble(), loc.getString("lng").toDouble()))} miles away")
        itemData.add(
